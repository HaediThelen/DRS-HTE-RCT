...
# "base_case" # Here we focus on the base_case trial setting
...
file_path <- sub("main_analysis", "LOO", file_path) # update file path
...
tx_intercept <- cx_intercept + log(0.8)
cov_ORs_tx <- c(1, 1, 1, 1.4, 1.2, 1, 2, 1.5, 1, 2.5, 2, 1.5) # Include ineractions
set.seed(3456)
...
tx_intercept <- cx_intercept + log(0.5)
cov_ORs_tx <- c(1, 1, 1, 1.4, 1.2, 1, 2, 1.5, 1, 2.5, 2, 1.5) # Include HTE
...
data_TxSc5_sum <- data_summary(data_TxSc5, "OR = 0.5 + HTE")
data_TxSc5_sum
...
# Parallel
system.time(TxSc1 <- simulation_function_LOO_parallel(data_TxSc1, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata", obj_strata_cuts = obj_strata_cuts))
TxSc2 <- simulation_function_LOO_parallel(data_TxSc2, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata", obj_strata_cuts)
TxSc3 <- simulation_function_LOO_parallel(data_TxSc3, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata", obj_strata_cuts)
TxSc4 <- simulation_function_LOO_parallel(data_TxSc4, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata", obj_strata_cuts)
TxSc5 <- simulation_function_LOO_parallel(data_TxSc5, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata", obj_strata_cuts)

# Save results as csv
write.csv(TxSc1, "./results/LOO/base_case/TxSc1.csv", row.names = FALSE)
write.csv(TxSc2, "./results/LOO/base_case/TxSc2.csv", row.names = FALSE)
write.csv(TxSc3, "./results/LOO/base_case/TxSc3.csv", row.names = FALSE)
write.csv(TxSc4, "./results/LOO/base_case/TxSc4.csv", row.names = FALSE)
write.csv(TxSc5, "./results/LOO/base_case/TxSc5.csv", row.names = FALSE)
