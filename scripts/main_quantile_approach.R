...
source("./functions/alt_quantile_simulation_functions.R")
...
# Change File Path for Ranking Approach
file_path <- sub("main_analysis", "quantile_analysis", file_path)
...
# Define objective_strata using quartiles but leaving ties together.
# There is no change from main_analysis.R for defining the strata for the 
# base population, just pointing out what happens.
data_TxSc1$objective_strata <- stratify_function(data_TxSc1, "pY0", 4) 
...
data_TxSc2$objective_strata <- stratify_function(data_TxSc2, "pY0", 4)
...
data_TxSc3$objective_strata <- stratify_function(data_TxSc3, "pY0", 4)
...
data_TxSc4$objective_strata <- stratify_function(data_TxSc4, "pY0", 4)
...
data_TxSc5$objective_strata <- stratify_function(data_TxSc5, "pY0", 4)
...
# use simulation_function_parallel_rank
TxSc1 <- simulation_function_parallel_rank(data_TxSc1, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata")
TxSc2 <- simulation_function_parallel_rank(data_TxSc2, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata")
TxSc3 <- simulation_function_parallel_rank(data_TxSc3, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata")
TxSc4 <- simulation_function_parallel_rank(data_TxSc4, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata")
TxSc5 <- simulation_function_parallel_rank(data_TxSc5, "Y", covs, n_sim, n_sample_cx, n_sample_tx, true_risk_strata = "objective_strata")
...
#add column for auc with auc in it
averages$AUC <- auc
averages
...
point_bias_plot <- composite_pointplot_bias(c_rd_data_short, small_titles,
                                            overall_title = paste0("Ranking Approach ", trial_setting),
