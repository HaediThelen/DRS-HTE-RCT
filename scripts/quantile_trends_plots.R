...
# This script generates supplemental figure B2 and B3: Trends Trends in bias as
# population parameters vary in the small treatment effect scenario (OR = 0.8) 
# without (B2) and with (B3) interactions, using the quantile analysis approach. 
...
plot_OPB <- function(data_list, x_variable, plot_titles = NULL, x_lab) {
  ...
  # Set y-max
  #y_max <- max(data_long$OPB, na.rm = TRUE)
  y_max <- 130
...
base_case_OPB <- read.csv("./results/quantile_analysis/base_case/base_case_OPB_results.csv")
a_samplesz500_OPB <- read.csv("./results/quantile_analysis/a_smplsz500/a_smplsz500_OPB_results.csv")
b_samplesz1000_OPB <- read.csv("./results/quantile_analysis/b_smplsz1000/b_smplsz1000_OPB_results.csv")
c_samplesz5000_OPB <- read.csv("./results/quantile_analysis/c_smplsz5000/c_smplsz5000_OPB_results.csv")
d_samplesz10000_OPB <- read.csv("./results/quantile_analysis/d_smplsz10000/d_smplsz10000_OPB_results.csv")
...
# Load data
base_case_OPB <- read.csv("./results/quantile_analysis/base_case/base_case_OPB_results.csv")
e_outcome_0.15_OPB <- read.csv("./results/quantile_analysis/e_outcome_0.15/e_outcome_0.15_OPB_results.csv")
f_outcome_0.05_OPB <- read.csv("./results/quantile_analysis/f_outcome_0.05/f_outcome_0.05_OPB_results.csv")
g_outcome_0.5_OPB <- read.csv("./results/quantile_analysis/g_outcome_0.5/g_outcome_0.5_OPB_results.csv")
...
base_case_OPB <- read.csv("./results/quantile_analysis/base_case/base_case_OPB_results.csv")
h_ratio_2to1_OPB <- read.csv("./results/quantile_analysis/h_ratio_2to1/h_ratio_2to1_OPB_results.csv")
i_ratio_3to1_OPB <- read.csv("./results/quantile_analysis/i_ratio_3to1/i_ratio_3to1_OPB_results.csv")
j_ratio_4to1_OPB <- read.csv("./results/quantile_analysis/j_ratio_4to1/j_ratio_4to1_OPB_results.csv")
...
# OPB vs AUC
base_case_OPB <- read.csv("./results/quantile_analysis/base_case/base_case_OPB_results.csv")
k_auc_0.70_OPB <- read.csv("./results/quantile_analysis/k_auc_0.70/k_auc_0.70_OPB_results.csv")
l_auc_0.75_OPB <- read.csv("./results/quantile_analysis/l_auc_0.75/l_auc_0.75_OPB_results.csv")
m_auc_0.80_OPB <- read.csv("./results/quantile_analysis/m_auc_0.80/m_auc_0.80_OPB_results.csv")
n_auc_0.85_OPB <- read.csv("./results/quantile_analysis/n_auc_0.85/n_auc_0.85_OPB_results.csv")
...
# B2 Supplement Figure of OR=0.8 no IXN
OPB_plots_summary_O8 <-ggarrange(OPB_plots_sample_size[[1]],
                                 OPB_plots_outcome_prop[[1]] + theme(axis.title.y = element_text(color = "transparent")),
                                 OPB_plots_rand_ratio[[1]],
                                 OPB_plots_auc[[1]]+ theme(axis.title.y = element_text(color = "transparent")),
                                 ncol = 2, nrow = 2, common.legend = TRUE, legend = "bottom",
                                 labels = "AUTO",
                                 font.label = list(size = 9)
)

OPB_plots_summary_O8 <- annotate_figure(OPB_plots_summary_O8, top = text_grob("Treatment Effect: OR = 0.8, Quantile Analysis", 
                                                                              color = "Black", size = 14))
OPB_plots_summary_O8
ggsave("./results/quantile_analysis/OPBOR08.pdf", plot = OPB_plots_summary_O8, 
       width = 6.5, height = 6.5, units = "in")

# B3 Supplement Figure of OR=0.8 with IXN
OPB_plots_summary_O8ixn <-ggarrange(OPB_plots_sample_size[[3]],
...
OPB_plots_summary_O8ixn <- annotate_figure(OPB_plots_summary_O8ixn, 
top = text_grob("Treatment Effect: OR = 0.8 + Covariate Interactions, Quantile Analysis", color = "Black", size = 14))
OPB_plots_summary_O8ixn 

ggsave("./results/quantile_analysis/OPBOR08ixn.pdf", plot = OPB_plots_summary_O8ixn, 
       width = 6.5, height = 6.5, units = "in")
