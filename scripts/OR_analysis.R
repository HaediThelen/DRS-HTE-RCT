...
source("./functions/or_function.R")
...
source("./functions/results_plots_OR.R")
...
# "z_smplsz100000"  "inverted_risk" "base_case_or"

current_trial <- all_trial_settings[all_trial_settings$trial_setting == "base_case_or",]
...
data_TxSc1_sum <- data_summary_or(data_TxSc1, "OR=1")
data_TxSc1_sum 
...
data_TxSc2_sum <- data_summary_or(data_TxSc2, "OR =0.8")
data_TxSc2_sum 
...
data_TxSc3_sum <- data_summary_or(data_TxSc3, "OR = 0.5")
data_TxSc3_sum 
...
data_TxSc4_sum <- data_summary_or(data_TxSc4, "OR = 0.8 + Interactions")
data_TxSc4_sum
...
data_TxSc5_sum <- data_summary_or(data_TxSc5, "OR = 0.5 + Interactions")
data_TxSc5_sum
...
TxSc1 <- simulation_function_parallel_or(data_TxSc1, "Y", covs, n_sim, n_sample_cx, n_sample_tx, 
                                      true_risk_strata = "objective_strata", obj_strata_cuts = obj_strata_cuts)
TxSc2 <- simulation_function_parallel_or(data_TxSc2, "Y", covs, n_sim, n_sample_cx, n_sample_tx, 
                                      true_risk_strata = "objective_strata", obj_strata_cuts = obj_strata_cuts)
TxSc3 <- simulation_function_parallel_or(data_TxSc3, "Y", covs, n_sim, n_sample_cx, n_sample_tx, 
                                      true_risk_strata = "objective_strata", obj_strata_cuts = obj_strata_cuts)
TxSc4 <- simulation_function_parallel_or(data_TxSc4, "Y", covs, n_sim, n_sample_cx, n_sample_tx, 
                                      true_risk_strata = "objective_strata", obj_strata_cuts = obj_strata_cuts)
TxSc5 <- simulation_function_parallel_or(data_TxSc5, "Y", covs, n_sim, n_sample_cx, n_sample_tx, 
                                      true_risk_strata = "objective_strata", obj_strata_cuts = obj_strata_cuts)
...
formatted_table_short <- c_sum_table_short %>%
  ...
  kable(
    col.names = c("Strata", "Event", "N", "Risk", "Event", "N", "Risk", "RD", "OR"),
    align = c('l', rep('c', 8)),
    caption = "Summary of Outcomes by Scenario, Objective Strata, and Treatment"
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  add_header_above(c(" " = 1, "Control" = 3, "Treatment" = 3, " " = 2)) %>%
...
TxSc1_bias <- log_or_bias_function(TxSc1)
TxSc2_bias <- log_or_bias_function(TxSc2)
TxSc3_bias <- log_or_bias_function(TxSc3)
TxSc4_bias <- log_or_bias_function(TxSc4)
TxSc5_bias <- log_or_bias_function(TxSc5)
c_log_or_data_short <- list(TxSc1_bias,TxSc2_bias, TxSc3_bias, TxSc4_bias, TxSc5_bias)

# save bias data
# write.csv(TxSc1_bias, paste0(file_path, "/", trial_setting, "_TxSc1_bias_or.csv"), row.names = FALSE)
# write.csv(TxSc2_bias, paste0(file_path, "/", trial_setting, "_TxSc2_bias_or.csv"), row.names = FALSE)
# write.csv(TxSc3_bias, paste0(file_path, "/", trial_setting, "_TxSc3_bias_or.csv"), row.names = FALSE)
# write.csv(TxSc4_bias, paste0(file_path, "/", trial_setting, "_TxSc4_bias_or.csv"), row.names = FALSE)
# write.csv(TxSc5_bias, paste0(file_path, "/", trial_setting, "_TxSc5_bias_or.csv"), row.names = FALSE)

# Point Plot of bias
methods <- c("CO", "FS", "SS")
sim_results <-list(TxSc1, TxSc2, TxSc3, TxSc4, TxSc5)
OPB_results <- apply_percent_bias_function(sim_results, methods, "True_log_or_strata")
OAB_results <- apply_OAB_function(sim_results, methods, "True_log_or_strata")
...
point_bias_plot <- composite_pointplot_bias(c_log_or_data_short, small_titles,
                                            overall_title = trial_setting,
                                            OPB_results, OAB_results)
point_bias_plot
# Save point plot of bias
# ggsave(paste0(file_path, "/", trial_setting, "_Point_Bias_Results.pdf"), plot = point_bias_plot, width = 6.5, height = 10, units = "in")